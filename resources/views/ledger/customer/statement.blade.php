...
@section('title')
    Customer Statement - {{ $member->nama }}
@endsection

@section('breadcrumb')
    @parent
    <li><a href="{{ route('member.index') }}">Customers</a></li>
    <li class="active">{{ $member->nama }} Statement</li>
@endsection
...
<style>
    ...
    @media print {
        ...
        .kpi-stat-card, .customer-header-box, .statement-table-box {
    ...
    <div class="col-lg-12">
        ...
        <div class="customer-header-box">
            <div style="display: flex; align-items: center; gap: 16px; flex-wrap: wrap;">
                ...
                <div>
                    <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap; margin-bottom: 6px;">
                        <span style="font-size: 18px; font-weight: 900; color: #0f172a;">{{ $member->nama }}</span>
                        @if($member->kode_member)
                            <span class="label label-success" style="font-size: 11px; font-weight: 800; border-radius: 4px; padding: 3px 8px;">{{ $member->kode_member }}</span>
                        @endif
                        @if($currentTotalDue > 0.01)
                            <span class="label label-danger" style="font-size: 11px; font-weight: 800; border-radius: 4px; padding: 3px 8px;">
                                <i class="fa fa-exclamation-circle"></i> Outstanding Due: {{ format_currency($currentTotalDue) }}
                            </span>
                        @else
                            <span class="label label-success" style="font-size: 11px; font-weight: 800; border-radius: 4px; padding: 3px 8px;">
                                <i class="fa fa-check-circle"></i> Account Clear
                            </span>
                        @endif
                    </div>
                    <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                        <span class="info-chip"><i class="fa fa-phone text-muted"></i> {{ $member->telepon ?: 'No Phone' }}</span>
                        <span class="info-chip"><i class="fa fa-map-marker text-muted"></i> {{ $member->alamat ?: 'No Address' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div style="display: flex; align-items: center; gap: 8px;" class="btn-print-hide">
                @if($currentTotalDue > 0.01)
                    <button type="button" class="btn btn-success btn-flat" style="border-radius: 8px; font-weight: 700; background-color: #10b981; border-color: #10b981; padding: 7px 16px;" onclick="openReceiveModal()">
                        <i class="fa fa-check-circle"></i> Receive Payment
                    </button>
                @endif
                ...
                <a href="{{ route('member.index') }}" class="btn btn-default btn-flat" style="border-radius: 8px; font-weight: 600; padding: 7px 14px; border-color: #cbd5e1;">
        ...
        <div class="ledger-filter-card">
            <form action="{{ route('ledger.customer.statement', $member->id_member) }}" method="GET" id="statementFilterForm">
        ...
        <div class="row">
            ...
            <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
                <div class="kpi-stat-card">
                    ...
                    <div>
                        ...
                        <div class="kpi-help-text">Prior due before period</div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
                <div class="kpi-stat-card">
                    ...
                    <div>
                        <div class="kpi-title-text" style="color: #7c3aed;">Total Invoiced (+)</div>
                        <div class="kpi-amount-text" style="color: #7c3aed;">{{ format_currency($invoicedInPeriod) }}</div>
                        <div class="kpi-help-text">Order bills in period</div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
                <div class="kpi-stat-card">
                    ...
                    <div>
                        <div class="kpi-title-text" style="color: #15803d;">Paid / Received (-)</div>
                        <div class="kpi-amount-text" style="color: #10b981;">{{ format_currency($receivedInPeriod) }}</div>
                        <div class="kpi-help-text">Payments collected</div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-6 col-sm-6 col-xs-12">
                <div class="kpi-stat-card">
                    ...
                    <div>
                        ...
                        <div class="kpi-help-text">Ending receivable due</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="statement-table-box">
            <div style="background: #f8fafc; border-bottom: 1px solid #e2e8f0; padding: 14px 20px; display: flex; justify-content: space-between; align-items: center;">
                <h4 style="margin: 0; font-weight: 800; font-size: 15px; color: #0f172a;">
                    <i class="fa fa-list-alt text-purple"></i> Account Statement Transactions
                </h4>
            ...
            <div class="table-responsive" style="padding: 15px 20px;">
                <table class="table table-striped table-bordered table-hover datatable-statement" style="width: 100%; margin-bottom: 0;">
                    <thead style="background: #f8fafc;">
                        ...
                        <th>Details / Notes</th>
                        <th style="color: #7c3aed; text-align: right;">Bill Added (+)</th>
                        <th style="color: #15803d; text-align: right;">Payment Received (-)</th>
                    ...
                    <tbody>
                        ...
                        <tr style="background: #f8fafc; font-weight: 700;">
                            ...
                            <td class="text-muted">Balance prior to {{ date('d M Y', strtotime($tanggalAwal)) }}</td>
                        ...
                        @forelse($transactions as $idx => $t)
                            <tr>
                                ...
                                <td>
                                    @if($t['credit'] > 0)
                                        <span class="label label-success" style="font-size: 10.5px; font-weight: 700;"><i class="fa fa-arrow-down"></i> Payment Received</span>
                                    @else
                                        <span class="label label-primary" style="font-size: 10.5px; font-weight: 700; background-color: #8b5cf6;"><i class="fa fa-file-text-o"></i> Sales Invoice</span>
                                    @endif
                                </td>
                                <td>{{ $t['description'] }}</td>
                                <td style="color: #7c3aed; font-weight: 700; text-align: right;">
                                ...
                                <td class="btn-print-hide text-center">
                                    <a href="{{ $t['print_url'] }}" target="_blank" class="btn btn-xs btn-default btn-flat" title="Print Invoice Slip" style="border-radius: 4px;">
                    ...
                    <tfoot style="background: #f8fafc; font-weight: 800;">
                        <tr>
                            ...
                            <td style="color: #7c3aed; text-align: right;">{{ format_currency($invoicedInPeriod) }}</td>
                            <td style="color: #15803d; text-align: right;">{{ format_currency($receivedInPeriod) }}</td>
        ...
        <form id="form-receive-customer" method="post" class="form-horizontal">
            ...
            <div class="modal-content" style="border-radius: 12px; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.2);">
                ...
                <div class="modal-body" style="padding: 20px;">
                    <input type="hidden" name="id_member" id="receive_id_member" value="{{ $member->id_member }}">
                    ...
                    <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px; margin-bottom: 15px;">
                        <div style="display:flex; justify-content:space-between; margin-bottom: 4px;">
                            ...
                            <strong style="color:#1e293b; font-size:12px;">{{ $member->nama }}</strong>
                        </div>
                    ...
                    <div class="form-group" style="margin-bottom: 12px;">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" step="any" min="1" max="{{ $currentTotalDue }}" name="amount" id="amount_received" class="form-control" value="{{ $currentTotalDue }}" required style="font-weight:700; font-size:15px; color:#0f172a; height:42px;">
                        </div>
                    </div>
